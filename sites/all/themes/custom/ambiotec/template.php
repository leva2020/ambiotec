...
/**
 * @file
 * Contains functions to alter Drupal's markup for the ambiotec theme.
 *
 *        _d^^^^^^^^^b_
 *     .d''           ``b.
 ...
 *   \__
 *
 * The ambiotec theme is a base theme designed to be easily extended by sub
 * themes. You should not modify this or any other file in the ambiotec theme
 ...
 *   http://drupal.org/project/ambiotec
 */
...
function ambiotec_preprocess_maintenance_page(&$vars, $hook) {
  // When a variable is manipulated or added in preprocess_html or
  // preprocess_page, that same work is probably needed for the maintenance page
  // as well, so we can just re-use those functions to do that work here.
  ambiotec_preprocess_html($vars);
}

function ambiotec_process_maintenance_page(&$vars, $hook) {
  // When a variable is manipulated or added in preprocess_html or
  // preprocess_page, that same work is probably needed for the maintenance page
  // as well, so we can just re-use those functions to do that work here.
  ambiotec_process_html($vars);
}
 ...
 */
function ambiotec_preprocess_html(&$vars) {
  // Viewport!
  $viewport = array(
    '#tag' => 'meta',
    '#attributes' => array(
  ...
  $vars['minie'] = theme_get_setting('ambiotec_min_ie_support');
  ...
  //////////////////////////////
  $livereload_port = theme_get_setting('ambiotec_livereload');
  if ($livereload_port) {
    if ($livereload_port == 'snugug') {
      $livereload_port = theme_get_setting('ambiotec_livereload_port');
 ...
 */
function ambiotec_process_html(&$vars) {

  //////////////////////////////
  // RWD Debug Integration
  //////////////////////////////
  if (theme_get_setting('ambiotec_viewport_width') || theme_get_setting('ambiotec_modernizr_debug')) {

    $debug_output = '<div id="ambiotec-debug">';

    if (theme_get_setting('ambiotec_viewport_width')) {
      $debug_output .= '<div id="ambiotec-viewport-width"></div>';
    }
    if (theme_get_setting('ambiotec_modernizr_debug')) {
      $debug_output .= '<div id="ambiotec-modernizr-debug" class="open"></div>';
    }
 ...
 */
function ambiotec_breadcrumb(&$vars) {
...
/**
  ...
  */
function ambiotec_process_html_tag(&$vars) {
  if (theme_get_setting('ambiotec_html_tags')) {
    $el = &$vars['element'];
 ...
 */
function ambiotec_preprocess_comment(&$vars) {
 ...
 */
function ambiotec_preprocess_user_profile_category(&$vars) {
 ...
 *
 */
function ambiotec_css_alter(&$css) {
  // Path to the theme's CSS directory.
  $dir = drupal_get_path('theme', 'ambiotec') . '/css';
  ...
  if (theme_get_setting('ambiotec_livereload')) {
 ...
 */
function ambiotec_preprocess_node(&$vars) {
  // Add article ARIA role.
  $vars['attributes_array']['role'] = 'article';
}

function ambiotec_preprocess_panels_pane(&$vars) {
...
function ambiotec_panels_default_style_render_region($vars) {
  $output = '';
  $output .= implode("\n", $vars['panes']);
  return $output;
}
...
//   */
// function ambiotec_blockify_logo(&$vars) {
//   return '';
// }

function ambiotec_preprocess_block(&$vars) {
  // Logo Block
