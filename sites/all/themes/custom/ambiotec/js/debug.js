jQuery(function($) {
  ...
  var viewportWidth = $('#ambiotec-viewport-width');
  var modernizrDebug = $('#ambiotec-modernizr-debug');
  ...
  if (viewportWidth.length) {
    ...
    if (supports_html5_storage()) {
      var viewportStatus = localStorage.getItem("ambiotec Viewport Storage");
    }
    ...
    if (viewportStatus !== null) {
      if (viewportStatus == 'px') {
        ...
        viewportWidth.html(widthEM + 'em');
        localStorage.setItem("ambiotec Viewport Storage", 'em');
      }
    }
    ...
    viewportWidth.click(function() {
      ...
      if (parseHTML == widthPX) {
        ...
        if (supports_html5_storage()) {
          localStorage.setItem("ambiotec Viewport Storage", 'px');
        }
      }
    });
  ...
  if (modernizrDebug.length) {
    if (typeof Modernizr === "undefined") {
      ...
      if (supports_html5_storage()) {
        var modernizrStatus = localStorage.getItem("ambiotec Modernizr Storage");
      }
      ...
      modernizrDebug.click(function() {
        if ($(this).hasClass('open')) {
          ...
          if (supports_html5_storage()) {
            localStorage.setItem("ambiotec Modernizr Storage", 'closed');
          }
        }
          ...
          if (supports_html5_storage()) {
            localStorage.setItem("ambiotec Modernizr Storage", 'open');
          }
        }
      });
    }
